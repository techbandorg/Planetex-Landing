...
export const getFormattedRoundAmounts = async () => {
  const { totalPurchaseAmount: usdtTotalPurchaseAmount, tokensSold: usdtTokensSold } = await getRoundAmounts(1)
  const { totalPurchaseAmount, tokensSold } = await getRoundAmounts(56)
  const purchaseSum = (BigInt(totalPurchaseAmount) + BigInt(usdtTotalPurchaseAmount)).toString()
